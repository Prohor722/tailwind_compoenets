...
export default function MoreAndMoreTableDesigns() {
  ...
  const data = [
    ...
    { id: 2, name: 'Mike Chen', dept: 'Engineering', email: 'mike@company.com', phone: '+1-555-0102', rating: 4.5, tasks: 18, time: '4:20 PM', trend: 'up', alerts: 0, verified: true },
    { id: 3, name: 'Emma Wilson', dept: 'Design', email: 'emma@company.com', phone: '+1-555-0103', rating: 4.9, tasks: 31, time: '2:15 PM', trend: 'down', alerts: 1, verified: false },
    { id: 4, name: 'James Johnson', dept: 'Analytics', email: 'james@company.com', phone: '+1-555-0104', rating: 4.6, tasks: 22, time: '5:30 PM', trend: 'up', alerts: 3, verified: true },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-blue-400 to-purple-400 mb-2">More Unique Tables</h1>
          <p className="text-slate-400">Advanced interactions and unique data visualizations</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-200">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900">Column Visibility Control</h2>
              <div className="flex gap-2">
                {['Name', 'Email', 'Phone', 'Tasks'].map((col, idx) => (
                  <button key={idx} onClick={() => toggleColumn(idx)} className={`flex items-center gap-1 px-3 py-1 rounded-lg text-sm font-medium transition-all ${visibleColumns.has(idx) ? 'bg-blue-100 text-blue-700' : 'bg-slate-100 text-slate-500'}`}>
                    {visibleColumns.has(idx) ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                    {col}
                  </button>
                ))}
              </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-100 border-b border-slate-200">
                  ...
                  {visibleColumns.has(1) && <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Email</th>}
                  {visibleColumns.has(2) && <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Phone</th>}
                  {visibleColumns.has(3) && <th className="px-6 py-4 text-center text-sm font-semibold text-slate-700">Tasks</th>}
                </tr>
              </thead>
              <tbody>
                {data.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 hover:bg-blue-50 transition-colors">
                    ...
                    {visibleColumns.has(1) && <td className="px-6 py-4 text-sm text-slate-600">{item.email}</td>}
                    {visibleColumns.has(2) && <td className="px-6 py-4 text-sm text-slate-600">{item.phone}</td>}
                    {visibleColumns.has(3) && <td className="px-6 py-4 text-center text-sm font-semibold text-slate-900">{item.tasks}</td>}
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-slate-700 overflow-hidden">
          <div className="p-6 border-b border-slate-700">
            <h2 className="text-lg font-semibold text-white">Status Indicator Rows</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-900">Trend Visualization</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-slate-100 to-blue-50 border-b border-slate-200">
                  <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Name</th>
                  <th className="px-6 py-4 text-left text-sm font-semibold text-slate-700">Tasks</th>
              ...
              <tbody>
                {data.map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-cyan-50 transition-all group cursor-pointer">
                    <td className="px-6 py-4 text-sm font-semibold text-slate-900 group-hover:text-blue-600">{item.name}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-24 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                          <div style={{ width: `${(item.tasks / 31) * 100}%` }} className="h-full bg-gradient-to-r from-blue-400 to-cyan-400"></div>
                        </div>
                        <span className="text-xs font-semibold text-slate-600 w-4">{item.tasks}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {item.trend === 'up' ? (
        ...
        <div>
          <h2 className="text-lg font-semibold text-white mb-4">Contact Card Layout</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.map((item) => (
              <div key={item.id} className="bg-slate-800 rounded-xl border border-slate-700 p-5 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/20 transition-all group cursor-pointer">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h3 className="text-white font-semibold group-hover:text-purple-300 transition-colors">{item.name}</h3>
                    <p className="text-sm text-slate-400">{item.dept}</p>
                  </div>
                  <div className={`w-3 h-3 rounded-full ${item.verified ? 'bg-emerald-500' : 'bg-slate-500'}`}></div>
                </div>
                
                <div className="space-y-2 mb-4 pb-4 border-b border-slate-700">
                  <div className="flex items-center gap-2 text-sm text-slate-300">
                    <Mail className="w-4 h-4 text-slate-500" />
                    <a href={`mailto:${item.email}`} className="hover:text-blue-400 transition-colors truncate">{item.email}</a>
                  </div>
                  <div className="flex items-center gap-2 text-sm text-slate-300">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-3 h-3 ${i < Math.floor(item.rating) ? 'fill-yellow-400 text-yellow-400' : 'text-slate-600'}`} />
                    ))}
                  </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="p-6 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-900">Icon-Rich Metrics</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center">
                        <div className="w-16 h-16 rounded-full bg-gradient-to-r from-emerald-400 to-blue-400 flex items-center justify-center relative">
                          <span className="text-white font-bold text-sm">{Math.round(item.rating * 10)}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center">
                        <div className="relative w-12 h-12">
                          <svg className="w-12 h-12 transform -rotate-90" viewBox="0 0 24 24">
                            ...
                            <circle cx="12" cy="12" r="10" fill="none" stroke="#3b82f6" strokeWidth="2" strokeDasharray={`${(item.rating / 5) * 62.8} 62.8`} strokeLinecap="round" />
                          </svg>
                          <span className="absolute inset-0 flex items-center justify-center text-xs font-bold text-slate-700">{Math.round((item.rating / 5) * 100)}%</span>
                        </div>
                      </div>
                    </td>
                  </tr>
