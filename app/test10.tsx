  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header */}
        <div className="mb-12">
          <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 mb-2">Even More Table Designs</h1>
          <p className="text-slate-400">Advanced interactions and professional layouts</p>
        </div>
        ...
        <div className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-2xl">
          ...
          <div className="divide-y divide-slate-800">
            {sortedData.map((item, idx) => (
              <div key={item.id} className="p-6 hover:bg-slate-800/50 transition-colors group cursor-pointer" onMouseEnter={() => setHoveredRow(item.id)} onMouseLeave={() => setHoveredRow(null)}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="text-white font-semibold group-hover:text-blue-300 transition-colors truncate">{item.title}</h3>
                      <span className={`inline-flex px-3 py-1 bg-gradient-to-r ${getStatusColor(item.status)} text-white text-xs font-bold rounded-full flex-shrink-0`}>
                        {item.status}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className="text-xs font-semibold text-slate-400">{item.priority}</span>
                    </div>
                  </div>

                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => toggleLike(item.id)} className={`p-2 rounded-lg transition-all ${likedRows.has(item.id) ? 'bg-red-500/20 text-red-400' : 'hover:bg-slate-700 text-slate-400'}`}>
                      <ThumbsUp className={`w-4 h-4 ${likedRows.has(item.id) ? 'fill-red-400' : ''}`} />
                    </button>
                    <button onClick={() => togglePin(item.id)} className={`p-2 rounded-lg transition-all ${pinnedRows.has(item.id) ? 'bg-yellow-500/20 text-yellow-400' : 'hover:bg-slate-700 text-slate-400 hover:text-slate-200'}`}>
                      {pinnedRows.has(item.id) ? <Pin className="w-4 h-4 fill-yellow-400" /> : <Pin className="w-4 h-4" />}
                    </button>
