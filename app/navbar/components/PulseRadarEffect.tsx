  ...
  return (
    <nav className="relative bg-black/80 backdrop-blur-xl rounded-3xl overflow-hidden border border-green-500/30">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
        <div
          className="absolute w-32 h-32 border-2 border-green-500/30 rounded-full animate-ping"
          style={{ animationDuration: "3s" }}
        ></div>
        <div
          className="absolute w-64 h-64 border-2 border-green-500/20 rounded-full animate-ping"
          style={{ animationDuration: "4s", animationDelay: "0.5s" }}
        ></div>
        <div
          className="absolute w-96 h-96 border-2 border-green-500/10 rounded-full animate-ping"
          ...
          <div className="lg:hidden mt-6 space-y-2 pb-4 border-t border-green-500/30 pt-4">
            {["SCAN", "DETECT", "TRACK", "ANALYZE"].map((item, idx) => (
              <a
                key={idx}
                href="#"
                className="block text-green-400/70 hover:text-green-400 py-2 px-4 rounded-lg hover:bg-green-500/10 transition-all font-mono font-bold"
              >
                {item}
              </a>
            ))}
