      ...
      <div className="space-y-4">
        ...
        <nav className="relative bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden">
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute -top-1/2 left-1/4 w-96 h-96 bg-purple-500/30 rounded-full blur-3xl animate-pulse" style={{animationDuration: '4s'}}></div>
            <div className="absolute top-1/2 right-1/4 w-96 h-96 bg-pink-500/30 rounded-full blur-3xl animate-pulse" style={{animationDuration: '6s', animationDelay: '1s'}}></div>
            <div className="absolute -bottom-1/2 left-1/2 w-96 h-96 bg-cyan-500/30 rounded-full blur-3xl animate-pulse" style={{animationDuration: '5s', animationDelay: '2s'}}></div>
          </div>
          ...
          <div className="relative max-w-7xl mx-auto px-6 py-5">
            <div className="flex items-center justify-between">
                ...
                {navItems.map((item, idx) => (
                  <button
                    ...
                    className={`relative px-6 py-2 rounded-full font-medium transition-all duration-500 ${
                      activeIndex === idx ? 'text-white z-10' : 'text-gray-400 hover:text-white'
                    }`}
                  >
                    {activeIndex === idx && (
                      <>
              ...
              <button onClick={() => setMobileMenu1(!mobileMenu1)} className="lg:hidden text-white p-2">
                <div className="relative w-6 h-6">
                  <span className={`absolute w-6 h-0.5 bg-white transition-all duration-300 ${mobileMenu1 ? 'rotate-45 top-3' : 'top-1'}`}></span>
                  <span className={`absolute w-6 h-0.5 bg-white top-3 transition-all duration-300 ${mobileMenu1 ? 'opacity-0' : 'opacity-100'}`}></span>
                  <span className={`absolute w-6 h-0.5 bg-white transition-all duration-300 ${mobileMenu1 ? '-rotate-45 top-3' : 'top-5'}`}></span>
                </div>
              </button>
            </div>
            ...
            {mobileMenu1 && (
              <div className="lg:hidden mt-6 space-y-3 pb-4 animate-in fade-in slide-in-from-top-4 duration-300">
                {navItems.map((item, idx) => (
                  <button key={idx} className="block w-full text-left text-white/90 hover:text-white py-3 px-4 rounded-lg hover:bg-white/10 transition-all">
                    {item}
                  </button>
                ))}
